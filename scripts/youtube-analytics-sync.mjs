...
async function report(token, channelId, startDate, params) {
...
for (const ch of channels) {
  ...
  try {
    ...
    const daily = await report(token, ch.youtube_channel_id, daysAgo(365), {
    ...
    const vids = await report(token, ch.youtube_channel_id, daysAgo(90), { dimensions: 'video', metrics: 'views,averageViewDuration', sort: '-views', maxResults: '10' });
    if (vids.length) {
    ...
    push(await report(token, ch.youtube_channel_id, daysAgo(28), { dimensions: 'insightTrafficSourceType', metrics: 'views,estimatedMinutesWatched', sort: '-views' }), 'traffic', (r) => ({ label: r[0], views: r[1], watch_minutes: r[2] }));
    push(await report(token, ch.youtube_channel_id, daysAgo(28), { dimensions: 'deviceType', metrics: 'views,estimatedMinutesWatched', sort: '-views' }), 'device', (r) => ({ label: r[0], views: r[1], watch_minutes: r[2] }));
    push(await report(token, ch.youtube_channel_id, daysAgo(28), { dimensions: 'subscribedStatus', metrics: 'views,estimatedMinutesWatched' }), 'subscribed', (r) => ({ label: r[0], views: r[1], watch_minutes: r[2] }));
    push(await report(token, ch.youtube_channel_id, daysAgo(28), { dimensions: 'ageGroup,gender', metrics: 'viewerPercentage', sort: '-viewerPercentage' }), 'age_gender', (r) => ({ label: r[0] + '|' + r[1], percent: r[2] }));
    ...
    console.error(`Analytics error for "${ch.name}":`, e.message);
  }
}

#!/usr/bin/env python3
"""Fetch Sleeper trending waiver drops and write data/sleeper-drops.json.

Mirrors scripts/sleeper_adds.py exactly, including the shared, cached player
map at data/sleeper-players.json (whichever job — adds or drops — runs first
in a given window refreshes it; the other reuses it via cache_is_fresh()).

Output is keyed by Sleeper player_id, same as sleeper-adds.json, with a
by_name fallback map for rows where Web_Data's sleeperId column is empty.
"""
...
TRENDING_URL = "https://api.sleeper.app/v1/players/nfl/trending/drop?lookback_hours={hours}&limit={limit}"

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DATA_DIR = os.path.join(ROOT, "data")
CACHE_PATH = os.path.join(DATA_DIR, "sleeper-players.json")
OUT_PATH = os.path.join(DATA_DIR, "sleeper-drops.json")
...
def main():
    ...
    print("Fetching trending drops ({}h lookback)".format(LOOKBACK_HOURS))
    ...
    ranked = sorted(players_out.items(), key=lambda kv: kv[1]["drops"], reverse=True)
    for i, (pid, row) in enumerate(ranked, start=1):
        players_out[pid]["drop_rank"] = i
    ...
    for pid, row in ranked[:10]:
        print("  {:>7,}  {} {} ({})  id={}".format(row["drops"], row["pos"], row["name"], row["team"], pid))
    return 0
